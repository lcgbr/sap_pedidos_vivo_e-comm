...
# dbutils.library.installPyPI("google-cloud-bigquery")
# dbutils.library.installPyPI("oauth2client")
# dbutils.library.installPyPI("google-api-python-client")
# dbutils.library.installPyPI("pandas", version="1.5.3")
# dbutils.library.installPyPI("db-dtypes")
...
# else:
start_date = subtract_days(0) 
  
# if (dbutils.widgets.get('end_date')):
#   end_date = dbutils.widgets.get('end_date')
# else:
end_date = subtract_days(0)
...
def auth():
    ...
    #response = requests.request("POST", url, headers={"User-Agent": "insomnia/8.4.5"})
...
token = auth()
#exit()
# token = {'access_token': 'Um5gfAyYb3uct06pc9oH-nWoTvo',
#  'token_type': 'bearer',
#  'expires_in': 311862,
#  'scope': 'openid'}
...
def selectBigQuery(dt):
    bigquery_client = bigquery.Client(credentials=CREDENTIALS)
  
    stmt = f"""
           select 
            distinct concat(numero_pedido,'|',replace(shop_code,'None','null')) numero_pedido_shopcode
            --distinct concat(numero_pedido) numero_pedido_shopcode
            FROM `telefonica-digitalsales.coe_datalake.sap_commerce_pedidos` where date(data_criacao_pedido) = '{dt}';
         """
...
#   dt_end = dt_end.replace('T', ' ')
  ...
  stmt = f"""
           delete FROM `telefonica-digitalsales.coe_datalake.sap_commerce_pedidos` where date(data_criacao_pedido) = '{dt_start}';
         """
  query_job = bigquery_client.query(stmt)

# COMMAND ----------

def delete_duplicados():
  bigquery_client = bigquery.Client(credentials=CREDENTIALS)
...
# getQuantityOrder('2022-11-27T00:00:00', '2022-11-27T23:59:59')
# list_orders = getOrders('2022-11-27T00:00:00', '2022-11-27T23:59:59')
  ...
  while page <= totalPages:
    ...
    try:
      ...
      response.drop(['__metadata','creationtime','date','user','paymentInfo','discounts',
                          ...
                          'deliveryStatus', 'salesApplication'], axis=1, inplace=True)
...
# start_date = '2022-11-24T00:00:00'
# end_date = '2022-11-24T23:59:59'
# list_orders = getOrders(start_date, end_date)
...
def getVendor(orderid):
    try:
        ...
        response = response.append({'numero_pedido':orderid.split('|')[0], 'login':0}, ignore_index=True)
    return response

# COMMAND ----------

def getOrder_deliverayDay(orderid):
...
def getSalesApplication(orderid):
...
def getOrdersDetails_coupon(orderid):
...
def getOrdersDetails_user(orderid):
...
# getOrdersDetails_user('22019876')
...
def getOrdersDetails_paymentInfo(orderid):
...
def getOrdersDetails_deliveryAddress(orderid):
  ...
  try:
    ...
    response = pd.DataFrame([response['d']])  
    response['numero_pedido'] = orderid.split('|')[0]
    response.drop(['creationtime', 'postalcode', 'streetnumber', 'town', 'complement', 'VAAddressCreationTime', 'streetname'],axis=1, inplace=True)
...
def getOrdersDetails_deliveryAddressRegion(orderid, idDelivery):
...
def getOrdersDetails_consignments(orderid):
...
# getOrdersDetails_consigments('0100071399|null')

# COMMAND ----------

def getOrdersDetails_consignmentsStatus(orderid, codigoRastreio):
...
def getOrdersDetails_paymentTransactions(orderid):
...
# getOrdersDetails_paymentTransactions('0100071399')
...
def getOrdersDetails_paymentTransactionsEntries(orderid,keyIntegration):
...
# getOrdersDetails_paymentTransactionsEntries(order, order_paymants_transaction['codigo_transacao'][0])
...
def getOrdersDetails_entries(orderid):
...
def getOrdersDetails_product(orderid, keyIntegration):
...
def getOrdersDetails_productSuperCatalog(orderid, keyIntegration):
  ...
  try:
    ...
    response = pd.DataFrame(columns={'orderid':orderid,'categoria_nvl_1':'NA'})
  return response
...
def getOrdersDetails_status(orderid):
...
def get_SkuDataSheet(sku):
        ...
        response_arr = {'marca':'', 'modelo':'','codigofabricante':'','ean':'', 'sku_produto':sku,'classificacao':''}
        ...
        response.rename(columns={'modelo':'nome_produto','marca':'nome_fabricante'}, inplace=True)
...
# list_orders['numero_pedido'].nunique()
...
def extract_sap():
  ...
  # end_date = '2023-07-24'
  start_date = datetime.now(timezone('America/Sao_Paulo')).strftime('%Y-%m-%d')
  end_date = datetime.now(timezone('America/Sao_Paulo')).strftime('%Y-%m-%d')

  current_date = date.fromisoformat(start_date)
  # loop request api for reprocess information
  # while current_date != date.fromisoformat(end_date):

  data_hora_request = datetime.now(timezone('America/Sao_Paulo')).strftime('%Y-%m-%d %H:%M:%S')
  ...
  while current_date == date.fromisoformat(end_date):
      ...
      list_orders_bq = selectBigQuery(date_start.replace('T03:00:00',''))
      list_orders = getOrders(date_start, date_end)

  #         list_orders.drop('page', axis=1, inplace=True)
      #print(list_orders.dtypes)
      list_orders = list_orders.drop_duplicates()
    
      if list_orders.empty:
        continue

      bq_unique = list_orders_bq.count().values[0]
      api_unique = list_orders['numero_pedido_shopcode'].nunique()
      contagem = 1

      print(f'BigQuery - {bq_unique}')
      print(f'API - {api_unique}')
      print(f'API - BigQuery = {api_unique - bq_unique}')

      if (api_unique - bq_unique) == 0:
          # print('Not found register - 10s minutes')          
          # exit
          print('Sem registros para inserir')
          return 0
      else:
        if list_orders.empty:
            # exit
            raise ValueError('list_orders vazio')
        else:
            for order in list_orders['numero_pedido_shopcode'].unique():
                if order not in list_orders_bq['numero_pedido_shopcode'].values:
                    dt_timestamp_ini = datetime.now()
                    # orders = list_orders[list_orders['numero_pedido_shopcode'] == order]
                    orders = list_orders[list_orders['numero_pedido_shopcode'] == order]
                    statusPedido = getOrdersDetails_status(order)
                    qtdDias = getOrder_deliverayDay(order)
                    order_user = getOrdersDetails_user(order)
                    order_cupom = getOrdersDetails_coupon(order)
                    order_paymants = getOrdersDetails_paymentInfo(order)
                    order_paymants_transaction = getOrdersDetails_paymentTransactions(order)
                    order_paymants_entries = getOrdersDetails_paymentTransactionsEntries(order, order_paymants_transaction['codigo_transacao'][0])
                   # order_paymants_entries = pd.DataFrame(order_paymants_entries)
                    order_list_product = getOrdersDetails_entries(order)
                    sapOrder = getOrdersDetails_consignments(order)
                    order_status_macro = getOrdersDetails_consignmentsStatus(order, sapOrder['codigo_rastreio'][0])
                    getlogin = getVendor(order)
                    order_sales_application = getSalesApplication(order)
                    order_delivery_status = getDeliveryStatus(order)
                    vivo_order_type = getVivoOrderType(order)
                    order_delivery_address = getOrdersDetails_deliveryAddress(order)
                    order_address_region = getOrdersDetails_deliveryAddressRegion(order, order_delivery_address['region_id'][0])

                    order_list_product_details = pd.DataFrame()
                    for key_integration in order_list_product['chave_integracao_produto']:
                      order_list_product_details_tmp = getOrdersDetails_product(order, key_integration)
        ...
        #               'ean',
                    ...
                    dt_timestamp_end = datetime.now()
                    print(f'{order} - {contagem} - {dt_timestamp_end - dt_timestamp_ini}')
                    contagem +=1                    

            if dataframe.empty:
                # exit
                raise ValueError('Dataframe final vazio')
            else:
  ...
  # DBTITLE 1,Exclui registros duplicados
  # delete_duplicados()
...
##### Main #####
# Chama a API do GA, executa as transformações e depois carrega no banco
log = {}

start_time = timeit.default_timer()
log['start_time'] = now_local().strftime('%Y-%m-%d %H:%M:%S')

log['report'] = f"Carga SAP Realtime - {now_local().strftime('%H:%M')}"
